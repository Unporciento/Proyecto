...
const VehicleModels = (function () {
  ...
  /**
   * Ronda 10 — Iluminación "Flat / Cel Shaded" (ver también scene3D.js,
   * buildLights). `MeshStandardMaterial` produce un degradado PBR
   * continuo entre luz y sombra: sutil en pantalla, pero la compresión
   * de video de Google Meet lo aplana en un manchón gris sin contraste.
   * `MeshToonMaterial` con un mapa de gradiente de pocos escalones fuerza
   * bandas de luz DISCRETAS (2-3 tonos, sin degradado intermedio) — el
   * mismo truco que usan los juegos "cel shaded" (Zelda Wind Waker,
   * Borderlands) para que la silueta se lea nítida incluso muy
   * comprimida. Se aplica solo a las superficies grandes y coloridas
   * (carrocería/cabina/tolva/acentos) que son las que más se benefician
   * del contraste; ruedas y detalles metálicos chicos se dejan con
   * material estándar (no aportan tanto y su reflejo especular ayuda a
   * distinguirlos del asfalto).
   */
  const toonGradientMap = (() => {
    ...
    // 3 escalones discretos: sombra / medio tono / luz plena.
    const shades = [70, 150, 235, 255];
    shades.forEach((v, i) => {
  ...
  /** Material "cel shaded" reutilizable para paneles grandes de carrocería. */
  function toonBodyMaterial(color, emissive, emissiveIntensity) {
  ...
  function addAxle(group, x, z, radius, width, trackWidth) {
    ...
    wheelR.position.set(x, radius, z - trackWidth / 2);
    group.add(wheelR);
  }
  ...
  function buildFurgon() {
    ...
    const bodyMat = toonBodyMaterial(c.body);
    ...
    const stripe = new THREE.Mesh(stripeGeo, toonBodyMaterial(c.accent, c.accent, 0.4));
  ...
  function buildBus() {
    ...
    const body = new THREE.Mesh(bodyGeo, toonBodyMaterial(c.body));
    ...
    const stripe = new THREE.Mesh(
      ...
      toonBodyMaterial(c.accent, c.accent, 0.4)
    );
  ...
  function buildMinero() {
    ...
    const hopper = new THREE.Mesh(hopperGeo, toonBodyMaterial(c.body));
    ...
    const stripe = new THREE.Mesh(
      ...
      toonBodyMaterial(c.accent, c.accent, 0.5)
    );
  ...
  function buildArticulado() {
    ...
    const cabin = new THREE.Mesh(cabinGeo, toonBodyMaterial(c.body));
  ...
  function buildGrua() {
    ...
    const turret = new THREE.Mesh(turretGeo, toonBodyMaterial(c.body));
    ...
    const boom = new THREE.Mesh(boomGeo, toonBodyMaterial(c.body));
    ...
    const counterweight = new THREE.Mesh(counterweightGeo, toonBodyMaterial(c.accent, c.accent, 0.25));
