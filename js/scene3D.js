...
const Scene3D = (function () {
  ...
  function buildImpactWall() {
    ...
    const redMat = new THREE.MeshStandardMaterial({ color: 0xe63946, roughness: 0.5, emissive: 0x3a0d10, emissiveIntensity: 0.15 });
    const whiteMat = new THREE.MeshStandardMaterial({ color: 0xf4f4f4, roughness: 0.5 });

    const halfSpan = ROAD_WIDTH / 2 - 0.4;
    let idx = 0;
    for (let x = -halfSpan; x <= halfSpan + 0.001; x += 1.6, idx++) {
      const mesh = new THREE.Mesh(blockGeo, (idx % 2 === 0) ? redMat.clone() : whiteMat.clone());
  ...
  function setImpactWallZ(z) {
    ...
    impactWallBlocks.forEach(({ mesh, restPos }, i) => {
      ...
      mesh.material = (i % 2 === 0) ? redMat : whiteMat;
    });
  ...
  function updateWeather(dt) {
    ...
    for (let i = 0; i < velocities.length; i++) {
      let y = posAttr.getY(i) - velocities[i] * dt;
      let x = posAttr.getX(i) + (kind === 'nieve' ? Math.sin((y + i) * 0.5) * drift * dt : 0);
      if (y < 0) {
        y = 40 + Math.random() * 5;
        x = (Math.random() - 0.5) * 90;
        posAttr.setZ(i, (Math.random() - 0.5) * (ROAD_LENGTH + 60));
      }
      posAttr.setX(i, x);
      posAttr.setY(i, y);
    }
