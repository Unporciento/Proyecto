...
const AudioEngine = (function () {
  ...
  let airBrakeHissTimeoutId = null; // permite cancelar el silbido si se detiene/reinicia antes de que suene
  ...
  function stopEngineLoop() {
    ...
    // Cancela el silbido de aire pendiente (ver triggerBrakeEngage): si el
    // usuario detiene/reinicia la simulación dentro de la ventana de 70ms,
    // evita que el silbido suene "fuera de contexto" sobre un vehículo
    // ya detenido/reiniciado.
    if (airBrakeHissTimeoutId !== null) {
      clearTimeout(airBrakeHissTimeoutId);
      airBrakeHissTimeoutId = null;
    }
  }
  ...
  function triggerBrakeEngage() {
    ...
    if (p.airBrake) {
      airBrakeHissTimeoutId = setTimeout(() => {
        airBrakeHissTimeoutId = null;
        playAirBrakeHiss();
      }, 70);
    }
